...
public class GameQty
{
  ...
  public GameQty(int aQty, int aId, Cart aCart, Game aGame)
  {
    ...
    if (!didAddCart)
    {
      throw new RuntimeException("Unable to create gameQty due to cart. See https://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
    boolean didAddGame = setGame(aGame);
    if (!didAddGame)
    {
      throw new RuntimeException("Unable to create gameQty due to game. See https://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
  }
