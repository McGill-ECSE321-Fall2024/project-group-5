...
public class Order
{
  ...
  public Order(int aOrderId, double aTotalPrice, boolean aIsPaid, boolean aDeliveryStatus, String aPromotionCode, boolean aUserAgreementCheck, PaymentInformation aPaymentInformation, CustomerAccount aCustomerAccount, Cart aCart, Address aAddress)
  {
    orderId = aOrderId;
    ...
    {
      throw new RuntimeException("Unable to create order due to paymentInformation. See https://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
    boolean didAddCustomerAccount = setCustomerAccount(aCustomerAccount);
    if (!didAddCustomerAccount)
    {
      throw new RuntimeException("Unable to create order due to customerAccount. See https://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
    ...
    if (!didAddAddress)
    {
      throw new RuntimeException("Unable to create order due to address. See https://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
  }
  ...
  public String toString()
  {
    return super.toString() + "["+
            ...
            "userAgreementCheck" + ":" + getUserAgreementCheck()+ "]" + System.getProperties().getProperty("line.separator") +
            "  " + "paymentInformation = "+(getPaymentInformation()!=null?Integer.toHexString(System.identityHashCode(getPaymentInformation())):"null") + System.getProperties().getProperty("line.separator") +
